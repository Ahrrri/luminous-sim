...
function canUseSkill(skill: any): boolean {
  ...
  // 쿨타임 체크
  if (simulation.cooldowns[skill.id] && simulation.cooldowns[skill.id] > 0) {
    return false;
  }

  // 진리의 문은 이퀼 당 1회만 사용 가능
  if (skill.id === 'DOOR_OF_TRUTH' && simulation.doorOfTruthUsed) {
    return false;
  }
  
  // 메모라이즈는 사용 가능 상태일 때만
  if (skill.id === 'MEMORIZE' && !simulation.memorizeAvailable) {
    return false;
  }
