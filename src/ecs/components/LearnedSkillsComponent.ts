...
export class LearnedSkillsComponent extends BaseComponent {
    ...
    updateFromEnhancements(enhancements: SkillEnhancement[], currentTime?: number): void {
        ...
        // 강화 기반 패시브 스킬들 습득 (액티브 스킬에 대해서만!)
        enhancements.forEach(enhancement => {
            // 기본 스킬이 실제로 존재하고 액티브 스킬인지 확인
            const baseSkill = LUMINOUS_SKILLS.find(s => s.id === enhancement.skillId);
            if (!baseSkill || baseSkill.category === 'passive_enhancement') {
                return; // 패시브 스킬은 강화하지 않음
            }

            // 5차 강화 → 리인포스 패시브
            if (enhancement.fifthLevel > 0 && baseSkill.canEnhanceFifth !== false) {
            ...
            // 6차 강화 → 마스터리 패시브
            if (enhancement.sixthLevel > 0 && baseSkill.canEnhanceSixth !== false) {
